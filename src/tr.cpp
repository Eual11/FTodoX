...
Component customFileReader(std::vector<std::string> DATA, std::string PATH,
                           const int _dim_x_, const int _dim_y_) {
  ...
  public:
    floatCTRL(std::size_t screen_dim, std::size_t content_len = 0)
        : step((content_len == 0)
                   ? 0.1f
                   : 1.0f / std::move(content_len)) // 0.1f if content_len is 0
    {
      if (content_len == 0) {
        min = 0.0f;
        max = 1.0f;
      } else {
        // the minimum value is obviously unstable. previous approach is
        // (step*screen_dim)/2 - (content_len % 2 != 0)?0:step/2) removed the
        // check and currently always reducing by half a step? Maybe it should
        // be half a step reduced while even, rather than the previous while odd
        // condition
    ...
    // walks the specified amount of steps to adjust the float_val, within the
    // bounds
    void walk(int steps) {
      this->float_val = bound(this->float_val + (this->step * steps));
    }
    // returns relative progress as float
    const float prog() { return (float_val - min) / (max - min); }
    // FUNCTOINS
  private:
    // returns input if it is in range, otherwise returns upper/lower boundary
    // respectively
    float bound(float input) const {
      return (std::min(max, std::max(min, input)));
    }
    // VARIABLES
  private:
  ...
  // change from std::vector<std::string> DATA to Elements. Allow for vector as
  // an alternate constructor
  class contentBase : public ComponentBase {
  public:
    // constructor
    explicit contentBase(std::vector<std::string> DATA,
                         std::filesystem::path PATH, const int _dim_y_,
                         const int _dim_x_)
        : _DATA(std::move(DATA)), _PATH(std::move(PATH)), float_x(_dim_x_),
          float_y(_dim_y_ - 4, _DATA.size()), dim_x(_dim_x_), dim_y(_dim_y_) {
      loadData();
    };
    //
    Element Render() override {
      return vbox(
          {hbox({text("<PATH: " + _PATH.relative_path().string() + ">") |
                     flex_grow | vcenter,
                 separatorEmpty(),
                 gaugeRight(float_y.prog()) |
                     size(Direction::WIDTH, Constraint::GREATER_THAN, 20) |
                     flex}),
           separator(),
      ...
      for (auto &str : _DATA) {
        ...
        if (strPos != std::string::npos and str.length() > 0)
          do {
            ...
            // if there are no more words, moves the strPos to the end of the
            // str the final portion of the string
            if (strPos == std::string::npos)
              strPos = str.length();
            // the word in the string
            auto sub_string =
                handleIndent(str.substr(lastPos, strPos - lastPos));
            if (sub_string.length() > 0) {
              ...
              if (cnt > 0) {
                ...
                for (int x = 0; x < cnt; x++) {
                  ...
                  std::size_t len =
                      std::min(dim_x, sub_string.length() - (dim_x * x));
                  // emlaces the sub string inside an element in the flex_string
                  flex_string.emplace_back(
                      text(sub_string.substr(pos_marker, len)));
                  // adjust position to represent where we will read from next
                  pos_marker += len;
                }
              } else
                flex_string.emplace_back(text(sub_string));
            }
            lastPos = strPos + 1;
          } while (strPos != std::string::npos);
        else
          flex_string.emplace_back(text(std::move(str)));
        //
        Component tmpComp = Renderer([&, row_num, total_places, flex_string] {
          return hbox({
              // if any string is added to this entry please adjust the SIZE
              // parameter to match
              vbox(hbox({
                       text(std::to_string(row_num)) | bold |
                           color(Color::Cyan) // before all adjustments
                           | align_right      // comes before size
                           | size(Direction::WIDTH, Constraint::EQUAL,
                                  total_places + 1), // after all adjustments
                       separator(),
                   }),
  ...
  return Make<contentBase>(std::move(DATA), std::move(PATH), std::move(_dim_y_),
                           std::move(_dim_x_));
};
...
int main(int argc, char **argv) {
  ...
  auto mainComp = customFileReader(std::move(file_contents), std::move(inP),
                                   comp_dim_x, comp_dim_y);
  mainComp |= border |
  ...
  screen.ExitLoopClosure(); // Unsure if this is necessary, treating it as a
                            // good luck charm
  return EXIT_SUCCESS;
}
/*TODO:
 * Allow for vim-like line jumping. E.g. typing `:60` will move the screen to
 * line 60 Allow for sed finding? Or just basic regex/rapidfuzz so that stuff
 * can be found easier in the doc. Maybe grep? Add command parsing Add a config
 * that contains default values(like the size of the window) or changes the
 * color scheme Adjust `screen` such that it fits within the terminal
 * boundaries(check if ftxui will provide the maximum possible dimension based
